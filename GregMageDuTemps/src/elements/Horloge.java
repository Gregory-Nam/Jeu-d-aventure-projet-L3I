...
public class Horloge extends Interactif {
	...
	/**
	 * La période aprés l'activation de l'horloge.
	 */
	private Periode periodeApresActivation;

	/*
	 * Tableau qui represente les items manquants de l'horloge. </br> Il doit étre
	 * rempli pour que l'horloge puisse étre activé.
	 */
	 ...
	 * @param nbItemManquant         nombre d'items é inserer pour activer
	 *                               l'horloge.
	 ...
	 */
	public Horloge(File image, Materiaux materiaux, int nbItemManquant, Periode periodeApresActivation,
			double position) {
	 ...
	 * Une interaction avec l'horloge est possible si celle-ci n'a pas déjé été
	 * activée; L'interaction avec l'horloge peu :
	 * <ul>
	...
	public void interagir() {
		...
		if (Jeu.getInstanceUnique().getPeriodeCourante() != periodeApresActivation.precedente()) {
			Jeu.getInstanceUnique().terminer("Tu n'étais pas dans le bon espace temps pour activer cet horloge...",
					false);
	...
	/**
	 * Renvoie si l'horloge peut étre activée.
	 * 
	 * @return vrai si l'horloge peut étre activée, faux sinon.
	 */
	...
	private int insererItemManquant(Item item) {
		if (!item.getMateriaux().equals(this.materiauxHorloge)) {
			Jeu.getInstanceUnique().terminer("L'horloge a été cassée par cet item en " + item.getMateriaux().toString(),
					false);
			return -1;
		}
