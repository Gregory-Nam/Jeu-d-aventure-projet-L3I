...
public class Jeu {
	...
	/**
	 * La largeur maximale de la fenétre.
	 */
	public static final int X_MAX_FENETRE = 1000;

	/*
	 * La largeur minimale de la fenétre.
	 */
	...
	/**
	 * La scéne de l'énigme.
	 */
	private Scene sceneEnigme;

	/**
	 * La scéne de la fin du jeu.
	 */
	 ...
	 * 
	 * @param stage le stage principale (la fenétre)
	 */
	...
	/**
	 * Permet d'initialiser la scéne de l'énigme.
	 */
	...
	/**
	 * Permet d'initialiser la scéne de l'inventaire.
	 */
	...
	/**
	 * Permet d'initialiser la scéne de fin.
	 * 
	...
	/**
	 * Permet d'initialiser la scéne du menu.
	 */
	...
	/**
	 * Permet l'initialisation ou la mise é jour des Interactif dans la vue du jeu.
	 */
	...
	/**
	 * Permet d'initialiser les PNJ et les items sur la premiére periode.
	 */
	private void initPnjItemPeriode1() {
		...
		PersonnageNonJoueur pnjBronze = new PersonnageNonJoueur(NomPNJ.KLACE_HEUREOUVERRE, 564, itemBronze,
				imagePourenigme, bas);
	...
	/**
	 * Permet d'initialiser les PNJ et les items sur la troisiéme période.
	 */
	...
	/**
	 * Permet de créer certains éléments (portes, salles, horloges) et d'initialiser
	 * les HashMap.
	 */
	private void initialisationDesElements() {
		...
		/* HORLOGES */
		Horloge horlogeBronze = new Horloge(new File("Images/Horloges/Horloge_bronze_transparence.png"),
				Materiaux.BRONZE, 1, Periode.PERIODE_2, 889);
		Horloge horlogeArgent = new Horloge(new File("Images/Horloges/Horloge_argent_transparence.png"),
				Materiaux.ARGENT, 1, Periode.PERIODE_3, 92);
		Horloge horlogeOr = new Horloge(new File("Images/Horloges/Horloge_or_transparence.png"), Materiaux.OR, 2,
				Periode.PERIODE_OBJECTIF, 853);
		HorlogePiege horlogePiege = new HorlogePiege(new File("Images/Horloges/Horloge_piege_transparence.png"),
				Materiaux.PLAQUE_OR, Periode.PERIODE_OBJECTIF, 86);
		...
		scene.setOnKeyPressed(new EventHandler<KeyEvent>() {
			...
			public void handle(KeyEvent event) {
				...
				case UP:
					...
					if (greg.getXMin() <= X_MIN_FENETRE || greg.getXMax() >= X_MAX_FENETRE)
						break;
	...
	/**
	 * Permet de lancer l'énigme du PNJ passé en paramétre.
	 * 
	...
	public void lancerEnigme(PersonnageNonJoueur pnj) {
		...
		rootEnigme.mettreEnActionChampsTextuel(() -> {
			...
			// expression reguliere : reponse du pnj précédé d'un espace ou rien et suivi
			// d'un espace, d'un point, d'une virgule, d'un point d'exclamation ou rien
			else if (Pattern.compile("(^|[ ])" + pnj.reponse() + "($|[.,! ])")
					.matcher(rootEnigme.getTexteDuChamps().toLowerCase()).find()) {
				/* La condition */
				rootEnigme.desactiverEntree();
			...
			public void handle(KeyEvent event) {
				...
				case ESCAPE:
					...
					if (itemSelectionne != null)
						greg.prendreItemEnMain(itemSelectionne);
					break;
				default:
					break;
				}
	...
	/**
	 * Permet de reprendre la partie la oé on était aprés avoir affiché le menu.
	 */
	 ...
	 * 
	 * @param message message é afficher
	 * @param seconde temps d'affichagee en seconde.
	 */
	public void afficheMessage(String message, double seconde) {
		this.message.setVisible(true);
		PauseTransition pause = new PauseTransition(Duration.seconds(seconde));
		pause.setOnFinished(event -> this.message.setVisible(false));
		pause.play();
